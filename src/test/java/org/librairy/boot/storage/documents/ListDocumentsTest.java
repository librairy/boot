...
//        udm.save(new Item());
//
        List<Resource> resources = udm.find(Resource.Type.DOCUMENT).all();
//
        LOG.info("Items: " + resources);

        List<Document> documents = resources.stream().map(resource -> udm.read(Resource.Type.DOCUMENT).byUri(resource.getUri()).get().asDocument()).filter(doc -> newFiles.contains(doc.getRetrievedFrom())).collect(Collectors.toList());
