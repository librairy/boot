...
import org.librairy.boot.model.domain.relations.SimilarToItems;
...
//                        udm.delete(Resource.Type.TOPIC).byUri(topicUri);
    ...
    public void transitiveRelationships() throws InterruptedException {
        ...
        // Words
        List<Word> words = IntStream.range(0, NUM_WORDS).mapToObj(i -> Resource.newWord(String.valueOf(i))).collect(Collectors.toList());
        words.forEach(word -> {
            udm.save(word);
        ...
        // Terms
        List<Term> terms = IntStream.range(0, NUM_TERMS).mapToObj(i -> Resource.newTerm(String.valueOf(i))).collect(Collectors.toList());
        terms.forEach(term -> {
            udm.save(term);
        ...
        // Topics
        List<Topic> docTopics = IntStream.range(0, NUM_DOC_TOPICS).mapToObj(i ->
                Resource.newTopic("d"+String.valueOf(i))).collect(Collectors.toList());
        ...
        List<Topic> itemTopics = IntStream.range(0, NUM_ITEM_TOPICS).mapToObj(i
                -> Resource.newTopic("i"+String.valueOf(i))).collect(Collectors
                .toList());
        ...
        List<Topic> partTopics = IntStream.range(0, NUM_PART_TOPICS).mapToObj(i
                -> Resource.newTopic("p"+String.valueOf(i))).collect(Collectors
                .toList());
        ...
        List<Document> documents    = IntStream.range(0, NUM_DOCUMENTS).mapToObj
                (i -> Resource.newDocument(String.valueOf(i))).collect(Collectors
                .toList
                ());
        documents.forEach(doc -> {
            ...
            // Items
            List<Item> internalItems    = IntStream.range(0, NUM_ITEMS).mapToObj
                    (i -> Resource.newItem(String.valueOf(i))).collect(Collectors
                    .toList());
            internalItems.forEach(item -> {
                ...
                // Parts
                List<Part> internalParts    = IntStream.range(0, NUM_PARTS)
                        .mapToObj(i -> Resource.newPart(String.valueOf(i))).collect
                                (Collectors.toList());
        ...
        udm.find(Relation.Type.EMERGES_IN).from(Resource.Type.DOMAIN,domainUri).forEach(emerges -> udm.delete(Resource.Type.TOPIC).byUri(emerges.getStartUri()));
        ...
        start = System.currentTimeMillis();
        udm.find(Relation.Type.APPEARED_IN).from(Resource.Type.DOMAIN,domainUri).forEach(appearedIn -> udm.delete(Resource.Type.TERM).byUri(appearedIn.getStartUri()));
        period = new Interval(start, System.currentTimeMillis()).toPeriod();
        ...
        start = System.currentTimeMillis();
        udm.find(Relation.Type.EMBEDDED_IN).from(Resource.Type.DOMAIN,domainUri).forEach(embeddedIn -> udm.delete(Resource.Type.WORD).byUri(embeddedIn.getStartUri()));
        period = new Interval(start, System.currentTimeMillis()).toPeriod();
    ...
    private void evaluate(){
        ...
        Assert.assertEquals(numDocs,            udm.count(Relation.Type.CONTAINS_TO_DOCUMENT).in(Resource.Type.DOMAIN,domainUri));
